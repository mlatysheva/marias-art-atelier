...
export default async function Paintings({ adminView = false }: PaintingsProps) {
    const paintings = adminView ? await getPaintingsOfLoggedUser() : await getPaintings();

    return (
      <>
      {adminView ? <Typography variant='h3' sx={{ mb: 3 }}>Manage your paintings</Typography> : <Typography variant='h3' sx={{ mb: 3 }}>Available paintings</Typography>}
